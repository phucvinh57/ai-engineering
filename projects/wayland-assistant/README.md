...
4. **Embed + store** — `ingest/embedder.py` batches chunk text through a local `sentence-transformers` model (no API calls); `ingest/store.py` upserts vectors + text + metadata into a persistent Chroma collection (`data/chroma/`), keyed by content hash for idempotent re-ingestion.
...
3. `rag/chat.py` streams a completion from the local LLM (via Ollama's OpenAI-compatible API) grounded in that context via `SYSTEM_PROMPT`, which enforces inline `[n]` citations, calls out protocol maturity, and asks a clarifying question when client-vs-compositor perspective is ambiguous. `search`/`/api/search` stop after step 2 (retrieval only, no LLM call).
