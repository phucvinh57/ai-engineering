...
context (`start_as_current_span` / contextvars). That sidesteps a real
hazard in this codebase: `POST /api/chat` streams its response through a
...
contextvar Context on *every single call*. A `with start_as_current_span():`
...
(see `langfuse/openai.py: OpenAiArgsExtractor`).
"""
...
class _NullSpan:
    ...
    def update_trace(self, **_kwargs: Any) -> _NullSpan:
        return self
...
def init(settings: Settings) -> None:
    ...
    that wants telemetry, e.g. `eval harvest`). No-ops when keys are absent."""
    global _client
    if _client is not None or not settings.telemetry_active:
        return
...
def shutdown() -> None:
    ...
    teardown. Bounded by `langfuse_timeout_seconds` -- a hung Langfuse must
    not hang Ctrl-C."""
    global _client
    if _client is None:
        return
...
def raw_client() -> Langfuse | None:
    """The underlying Langfuse SDK client, for callers that need its `.api`
    REST surface (e.g. eval/harvest.py reading traces back out). Everything
    in this module that only *writes* spans/scores should prefer the
    higher-level helpers above instead."""
    return _client


def new_trace_id() -> str | None:
    """A 32-char lowercase-hex OTel trace id, or None when inactive. Use
    this -- never `uuid4()` -- Langfuse v3 rejects a dashed UUID."""
    if _client is None:
        return None
    from langfuse import Langfuse
...
def start_root(
    ...
    *,
    ...
    if _client is None or trace_id is None:
        return _NULL_SPAN
    span = _client.start_span(trace_context={"trace_id": trace_id}, name=name, input=input, metadata=metadata)
    span.update_trace(session_id=session_id, user_id=user_id, tags=tags, input=input)
    return span
...
def make_openai_client(settings: Settings) -> Any:
    ...
    live request paths: the CLI and eval/runner.py never call `init()`, so
    they always get a vanilla client and never emit generation spans --
    scope is enforced by construction, not by discipline at call sites."""
    if _client is None:
        return VanillaOpenAI(base_url=settings.chat_base_url, api_key=settings.chat_api_key)
    from langfuse.openai import OpenAI as LangfuseOpenAI

    return LangfuseOpenAI(base_url=settings.chat_base_url, api_key=settings.chat_api_key)
...
def generation_kwargs(parent: Any, *, name: str) -> dict[str, Any]:
    ...
    `condense_query` accept an injected client, and eval/runner.py injects a
    vanilla one)."""
